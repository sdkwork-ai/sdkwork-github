...
fn ph(is_sqlite: bool, idx: usize) -> String {
    if is_sqlite {
        "?".to_string()
    } else {
        format!("${idx}")
...
impl TrackerStore for SqlGitHubStore {
    async fn list_tracker_issues(
        ...
        let is_sqlite = matches!(self.pool().engine(), DatabaseEngine::Sqlite);
        ...
        idx += 1;
        conditions.push(format!("tenant_id = {}", ph(is_sqlite, idx)));
        binds.push(tenant_id.to_string());
        idx += 1;
        conditions.push(format!("organization_id = {}", ph(is_sqlite, idx)));
        binds.push(organization_id.to_string());

        if let Some(ref t) = query.issue_type {
            idx += 1;
            conditions.push(format!("type = {}", ph(is_sqlite, idx)));
            binds.push(t.clone());
        }
        if let Some(ref s) = query.status {
            idx += 1;
            conditions.push(format!("status = {}", ph(is_sqlite, idx)));
            binds.push(s.clone());
        }
        if let Some(ref p) = query.priority {
            idx += 1;
            conditions.push(format!("priority = {}", ph(is_sqlite, idx)));
            binds.push(p.clone());
        }
        if let Some(ref m) = query.milestone_id {
            idx += 1;
            conditions.push(format!("milestone_id = {}", ph(is_sqlite, idx)));
            binds.push(m.clone());
        }
        if let Some(ref q) = query.q {
            idx += 1;
            let p = ph(is_sqlite, idx);
        ...
        idx += 1;
        let limit_p = ph(is_sqlite, idx);
        idx += 1;
        let offset_p = ph(is_sqlite, idx);
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                ...
                let mut sq = sqlx::query_as::<_, TrackerIssueRow>(&select_sql);
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                ...
                let row = sqlx::query_as::<_, TrackerIssueRow>(
                    &format!("SELECT {cols} FROM github_tracker_issue WHERE tenant_id = ? AND organization_id = ? AND id = ?"),
                )
            ...
            DatabaseEngine::Postgres => {
                let pool = self.pool().as_postgres().expect("postgres pool");
                let row = sqlx::query_as::<_, TrackerIssueRow>(
                    &format!("SELECT {cols} FROM github_tracker_issue WHERE tenant_id = $1 AND organization_id = $2 AND id = $3"),
                )
                .bind(tenant_id).bind(organization_id).bind(issue_id)
                .fetch_optional(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                row.map(Into::into).ok_or_else(|| ServiceError::NotFound(format!("tracker issue {issue_id} not found")))
    ...
    async fn create_tracker_issue(
        ...
        let sql = if is_sqlite {
            "INSERT INTO github_tracker_issue (id, tenant_id, organization_id, title, description, type, status, priority, submitted_by, assignee_id, milestone_id, github_issue_id, vote_count, comment_count, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, 'open', ?, ?, NULL, ?, NULL, 0, 0, ?, ?)"
        } else {
            "INSERT INTO github_tracker_issue (id, tenant_id, organization_id, title, description, type, status, priority, submitted_by, assignee_id, milestone_id, github_issue_id, vote_count, comment_count, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, $6, 'open', $7, $8, NULL, $9, NULL, 0, 0, $10, $11)"
        };
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                ...
                for lid in &command.label_ids {
                    sqlx::query("INSERT INTO github_tracker_issue_label (issue_id, label_id) VALUES (?, ?)")
                        .bind(&id).bind(lid)
                        .execute(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                }
            }
    ...
    async fn update_tracker_issue(
        ...
        if command.title.is_some() { idx += 1; sets.push(format!("title = {}", ph(is_sqlite, idx))); }
        if command.status.is_some() { idx += 1; sets.push(format!("status = {}", ph(is_sqlite, idx))); }
        if command.priority.is_some() { idx += 1; sets.push(format!("priority = {}", ph(is_sqlite, idx))); }
        if command.assignee_id.is_some() { idx += 1; sets.push(format!("assignee_id = {}", ph(is_sqlite, idx))); }
        if command.milestone_id.is_some() { idx += 1; sets.push(format!("milestone_id = {}", ph(is_sqlite, idx))); }
        idx += 1; sets.push(format!("updated_at = {}", ph(is_sqlite, idx)));
        idx += 1; let id_p = ph(is_sqlite, idx);
        idx += 1; let t_p = ph(is_sqlite, idx);
        idx += 1; let o_p = ph(is_sqlite, idx);
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                ...
                if let Some(ref label_ids) = command.label_ids {
                    sqlx::query("DELETE FROM github_tracker_issue_label WHERE issue_id = ?").bind(issue_id).execute(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                    for lid in label_ids {
                        sqlx::query("INSERT INTO github_tracker_issue_label (issue_id, label_id) VALUES (?, ?)").bind(issue_id).bind(lid).execute(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                    }
                }
            }
            DatabaseEngine::Postgres => {
    ...
    async fn list_labels_for_issue_ids(
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let placeholders = std::iter::repeat("?").take(issue_ids.len()).collect::<Vec<_>>().join(", ");
                let sql = format!("SELECT il.issue_id, l.id, l.name, l.color, l.description FROM github_tracker_issue_label il JOIN github_tracker_label l ON il.label_id = l.id WHERE il.issue_id IN ({placeholders})");
                let mut q = sqlx::query_as::<_, IssueLabelRow>(&sql);
    ...
    async fn get_milestone_for_issue(&self, milestone_id: &str) -> Result<Option<TrackerMilestone>, ServiceError> {
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let row = sqlx::query_as::<_, MilestoneRow>("SELECT id, title, description, status, due_date FROM github_tracker_milestone WHERE id = ?").bind(milestone_id).fetch_optional(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
    ...
    async fn list_tracker_comments(&self, issue_id: &str, page: u32, page_size: u32) -> Result<Page<TrackerComment>, ServiceError> {
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let (total,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM github_tracker_comment WHERE issue_id = ?").bind(issue_id).fetch_one(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                let rows = sqlx::query_as::<_, CommentRow>(&format!("SELECT {cols} FROM github_tracker_comment WHERE issue_id = ? ORDER BY created_at ASC LIMIT ? OFFSET ?")).bind(issue_id).bind(limit).bind(offset).fetch_all(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(Page { items: rows.into_iter().map(Into::into).collect(), page, page_size, total: total as u64 })
            }
            DatabaseEngine::Postgres => {
                let pool = self.pool().as_postgres().expect("postgres pool");
                ...
                let rows = sqlx::query_as::<_, CommentRow>(&format!("SELECT {cols} FROM github_tracker_comment WHERE issue_id = $1 ORDER BY created_at ASC LIMIT $2 OFFSET $3")).bind(issue_id).bind(limit).bind(offset).fetch_all(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(Page { items: rows.into_iter().map(Into::into).collect(), page, page_size, total: total as u64 })
            }
        }
    }
    ...
    async fn create_tracker_comment(&self, issue_id: &str, author_id: &str, content: &str) -> Result<TrackerComment, ServiceError> {
        ...
        let now = format_timestamp(Utc::now());
        let is_sqlite = matches!(self.pool().engine(), DatabaseEngine::Sqlite);
        let (ins_sql, upd_sql) = if is_sqlite {
            ("INSERT INTO github_tracker_comment (id, issue_id, author_id, content, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
             "UPDATE github_tracker_issue SET comment_count = comment_count + 1, updated_at = ? WHERE id = ?")
        } else {
            ("INSERT INTO github_tracker_comment (id, issue_id, author_id, content, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6)",
             "UPDATE github_tracker_issue SET comment_count = comment_count + 1, updated_at = $1 WHERE id = $2")
        };
        let cols = "id, issue_id, author_id, content, created_at, updated_at";
        let fetch_sql = if is_sqlite { format!("SELECT {cols} FROM github_tracker_comment WHERE id = ?") } else { format!("SELECT {cols} FROM github_tracker_comment WHERE id = $1") };
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                ...
                let row = sqlx::query_as::<_, CommentRow>(&fetch_sql).bind(&id).fetch_one(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(row.into())
            }
            DatabaseEngine::Postgres => {
                let pool = self.pool().as_postgres().expect("postgres pool");
    ...
    async fn toggle_tracker_vote(&self, issue_id: &str, user_id: &str) -> Result<bool, ServiceError> {
        ...
        let is_sqlite = matches!(self.pool().engine(), DatabaseEngine::Sqlite);
        let (del_sql, dec_sql, ins_sql, inc_sql) = if is_sqlite {
            ("DELETE FROM github_tracker_vote WHERE issue_id = ? AND user_id = ?",
             "UPDATE github_tracker_issue SET vote_count = MAX(vote_count - 1, 0), updated_at = ? WHERE id = ?",
             "INSERT INTO github_tracker_vote (id, issue_id, user_id, created_at) VALUES (?, ?, ?, ?)",
             "UPDATE github_tracker_issue SET vote_count = vote_count + 1, updated_at = ? WHERE id = ?")
        } else {
            ("DELETE FROM github_tracker_vote WHERE issue_id = $1 AND user_id = $2",
             "UPDATE github_tracker_issue SET vote_count = GREATEST(vote_count - 1, 0), updated_at = $1 WHERE id = $2",
             "INSERT INTO github_tracker_vote (id, issue_id, user_id, created_at) VALUES ($1, $2, $3, $4)",
             "UPDATE github_tracker_issue SET vote_count = vote_count + 1, updated_at = $1 WHERE id = $2")
        };
    ...
    async fn has_voted(&self, issue_id: &str, user_id: &str) -> Result<bool, ServiceError> {
        let sql = match self.pool().engine() {
            DatabaseEngine::Sqlite => "SELECT COUNT(*) FROM github_tracker_vote WHERE issue_id = ? AND user_id = ?",
            DatabaseEngine::Postgres => "SELECT COUNT(*) FROM github_tracker_vote WHERE issue_id = $1 AND user_id = $2",
        };
        let (count,): (i64,) = match self.pool().engine() {
            DatabaseEngine::Sqlite => sqlx::query_as(sql).bind(issue_id).bind(user_id).fetch_one(self.pool().as_sqlite().expect("sqlite pool")).await.map_err(|e| ServiceError::Repository(e.to_string()))?,
            DatabaseEngine::Postgres => sqlx::query_as(sql).bind(issue_id).bind(user_id).fetch_one(self.pool().as_postgres().expect("postgres pool")).await.map_err(|e| ServiceError::Repository(e.to_string()))?,
        };
        Ok(count > 0)
    }
    ...
    async fn list_tracker_labels(&self, tenant_id: &str, organization_id: &str) -> Result<Vec<TrackerLabel>, ServiceError> {
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let rows = sqlx::query_as::<_, LabelRow>("SELECT id, name, color, description FROM github_tracker_label WHERE tenant_id = ? AND organization_id = ? ORDER BY name ASC").bind(tenant_id).bind(organization_id).fetch_all(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(rows.into_iter().map(Into::into).collect())
            }
            DatabaseEngine::Postgres => {
                let pool = self.pool().as_postgres().expect("postgres pool");
    ...
    async fn create_tracker_label(&self, tenant_id: &str, organization_id: &str, name: &str, color: &str, description: Option<&str>) -> Result<TrackerLabel, ServiceError> {
        ...
        let now = format_timestamp(Utc::now());
        let sql = match self.pool().engine() {
            DatabaseEngine::Sqlite => "INSERT INTO github_tracker_label (id, tenant_id, organization_id, name, color, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            DatabaseEngine::Postgres => "INSERT INTO github_tracker_label (id, tenant_id, organization_id, name, color, description, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
        };
        match self.pool().engine() {
            DatabaseEngine::Sqlite => { let pool = self.pool().as_sqlite().expect("sqlite pool"); sqlx::query(sql).bind(&id).bind(tenant_id).bind(organization_id).bind(name).bind(color).bind(description).bind(&now).bind(&now).execute(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?; }
    ...
    async fn list_tracker_milestones(&self, tenant_id: &str, organization_id: &str, status: Option<&str>) -> Result<Vec<MilestoneProgress>, ServiceError> {
        ...
        let (where_sql, binds): (String, Vec<&str>) = match status {
            Some(s) => { let mut idx = 0; idx += 1; let p1 = ph(is_sqlite, idx); idx += 1; let p2 = ph(is_sqlite, idx); idx += 1; let p3 = ph(is_sqlite, idx); (format!("m.tenant_id = {p1} AND m.organization_id = {p2} AND m.status = {p3}"), vec![tenant_id, organization_id, s]) }
            None => { let mut idx = 0; idx += 1; let p1 = ph(is_sqlite, idx); idx += 1; let p2 = ph(is_sqlite, idx); (format!("m.tenant_id = {p1} AND m.organization_id = {p2}"), vec![tenant_id, organization_id]) }
        };
        let sql = format!(
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let mut q = sqlx::query_as::<_, MilestoneProgressRow>(&sql);
    ...
    async fn create_tracker_milestone(&self, tenant_id: &str, organization_id: &str, title: &str, description: Option<&str>, due_date: Option<&str>) -> Result<TrackerMilestone, ServiceError> {
        ...
        let now = format_timestamp(Utc::now());
        let sql = match self.pool().engine() {
            DatabaseEngine::Sqlite => "INSERT INTO github_tracker_milestone (id, tenant_id, organization_id, title, description, status, due_date, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'open', ?, ?, ?)",
            DatabaseEngine::Postgres => "INSERT INTO github_tracker_milestone (id, tenant_id, organization_id, title, description, status, due_date, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, 'open', $6, $7, $8)",
        };
        match self.pool().engine() {
            DatabaseEngine::Sqlite => { let pool = self.pool().as_sqlite().expect("sqlite pool"); sqlx::query(sql).bind(&id).bind(tenant_id).bind(organization_id).bind(title).bind(description).bind(due_date).bind(&now).bind(&now).execute(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?; }
    ...
    async fn get_tracker_milestone_issues(&self, tenant_id: &str, organization_id: &str, milestone_id: &str, page: u32, page_size: u32) -> Result<Page<TrackerIssue>, ServiceError> {
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let (total,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM github_tracker_issue WHERE tenant_id = ? AND organization_id = ? AND milestone_id = ?").bind(tenant_id).bind(organization_id).bind(milestone_id).fetch_one(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                let rows = sqlx::query_as::<_, TrackerIssueRow>(&format!("SELECT {cols} FROM github_tracker_issue WHERE tenant_id = ? AND organization_id = ? AND milestone_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?")).bind(tenant_id).bind(organization_id).bind(milestone_id).bind(limit).bind(offset).fetch_all(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(Page { items: rows.into_iter().map(Into::into).collect(), page, page_size, total: total as u64 })
            }
            DatabaseEngine::Postgres => {
                let pool = self.pool().as_postgres().expect("postgres pool");
    ...
    async fn list_tracker_roadmaps(&self, tenant_id: &str, organization_id: &str, page: u32, page_size: u32) -> Result<Page<TrackerRoadmap>, ServiceError> {
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let (total,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM github_tracker_roadmap WHERE tenant_id = ? AND organization_id = ?").bind(tenant_id).bind(organization_id).fetch_one(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                let rows = sqlx::query_as::<_, RoadmapRow>(&format!("SELECT {cols} FROM github_tracker_roadmap WHERE tenant_id = ? AND organization_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?")).bind(tenant_id).bind(organization_id).bind(limit).bind(offset).fetch_all(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(Page { items: rows.into_iter().map(Into::into).collect(), page, page_size, total: total as u64 })
            }
            DatabaseEngine::Postgres => {
                let pool = self.pool().as_postgres().expect("postgres pool");
                ...
                let rows = sqlx::query_as::<_, RoadmapRow>(&format!("SELECT {cols} FROM github_tracker_roadmap WHERE tenant_id = $1 AND organization_id = $2 ORDER BY created_at DESC LIMIT $3 OFFSET $4")).bind(tenant_id).bind(organization_id).bind(limit).bind(offset).fetch_all(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(Page { items: rows.into_iter().map(Into::into).collect(), page, page_size, total: total as u64 })
            }
        }
    }
    ...
    async fn create_tracker_roadmap(&self, tenant_id: &str, organization_id: &str, title: &str, description: Option<&str>, start_date: Option<&str>, target_date: Option<&str>) -> Result<TrackerRoadmap, ServiceError> {
        ...
        let now = format_timestamp(Utc::now());
        let sql = match self.pool().engine() {
            DatabaseEngine::Sqlite => "INSERT INTO github_tracker_roadmap (id, tenant_id, organization_id, title, description, status, start_date, target_date, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'active', ?, ?, ?, ?)",
            DatabaseEngine::Postgres => "INSERT INTO github_tracker_roadmap (id, tenant_id, organization_id, title, description, status, start_date, target_date, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, 'active', $6, $7, $8, $9)",
        };
        match self.pool().engine() {
            DatabaseEngine::Sqlite => { let pool = self.pool().as_sqlite().expect("sqlite pool"); sqlx::query(sql).bind(&id).bind(tenant_id).bind(organization_id).bind(title).bind(description).bind(start_date).bind(target_date).bind(&now).bind(&now).execute(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?; }
    ...
    async fn get_tracker_roadmap(&self, tenant_id: &str, organization_id: &str, roadmap_id: &str) -> Result<TrackerRoadmap, ServiceError> {
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let row = sqlx::query_as::<_, RoadmapRow>(&format!("SELECT {cols} FROM github_tracker_roadmap WHERE tenant_id = ? AND organization_id = ? AND id = ?")).bind(tenant_id).bind(organization_id).bind(roadmap_id).fetch_optional(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                row.map(Into::into).ok_or_else(|| ServiceError::NotFound(format!("roadmap {roadmap_id} not found")))
            }
            DatabaseEngine::Postgres => {
                let pool = self.pool().as_postgres().expect("postgres pool");
                let row = sqlx::query_as::<_, RoadmapRow>(&format!("SELECT {cols} FROM github_tracker_roadmap WHERE tenant_id = $1 AND organization_id = $2 AND id = $3")).bind(tenant_id).bind(organization_id).bind(roadmap_id).fetch_optional(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                row.map(Into::into).ok_or_else(|| ServiceError::NotFound(format!("roadmap {roadmap_id} not found")))
            }
        }
    }
    ...
    async fn update_tracker_roadmap(&self, tenant_id: &str, organization_id: &str, roadmap_id: &str, title: Option<&str>, description: Option<Option<&str>>, status: Option<&str>, start_date: Option<Option<&str>>, target_date: Option<Option<&str>>) -> Result<TrackerRoadmap, ServiceError> {
        ...
        let mut sets: Vec<String> = Vec::new();
        let mut idx = 0;
        if title.is_some() { idx += 1; sets.push(format!("title = {}", ph(is_sqlite, idx))); }
        if description.is_some() { idx += 1; sets.push(format!("description = {}", ph(is_sqlite, idx))); }
        if status.is_some() { idx += 1; sets.push(format!("status = {}", ph(is_sqlite, idx))); }
        if start_date.is_some() { idx += 1; sets.push(format!("start_date = {}", ph(is_sqlite, idx))); }
        if target_date.is_some() { idx += 1; sets.push(format!("target_date = {}", ph(is_sqlite, idx))); }
        idx += 1; sets.push(format!("updated_at = {}", ph(is_sqlite, idx)));
        idx += 1; let id_p = ph(is_sqlite, idx);
        idx += 1; let t_p = ph(is_sqlite, idx);
        idx += 1; let o_p = ph(is_sqlite, idx);
    ...
    async fn list_tracker_roadmap_items(&self, roadmap_id: &str) -> Result<Vec<TrackerRoadmapItem>, ServiceError> {
        ...
        match self.pool().engine() {
            DatabaseEngine::Sqlite => {
                let pool = self.pool().as_sqlite().expect("sqlite pool");
                let rows = sqlx::query_as::<_, RoadmapItemRow>(&format!("SELECT {cols} FROM github_tracker_roadmap_item WHERE roadmap_id = ? ORDER BY sort_order ASC")).bind(roadmap_id).fetch_all(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(rows.into_iter().map(Into::into).collect())
            }
            DatabaseEngine::Postgres => {
                let pool = self.pool().as_postgres().expect("postgres pool");
                let rows = sqlx::query_as::<_, RoadmapItemRow>(&format!("SELECT {cols} FROM github_tracker_roadmap_item WHERE roadmap_id = $1 ORDER BY sort_order ASC")).bind(roadmap_id).fetch_all(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?;
                Ok(rows.into_iter().map(Into::into).collect())
            }
        }
    }
    ...
    async fn add_tracker_roadmap_item(&self, roadmap_id: &str, command: &CreateTrackerRoadmapItemCommand) -> Result<TrackerRoadmapItem, ServiceError> {
        ...
        let sql = match self.pool().engine() {
            DatabaseEngine::Sqlite => "INSERT INTO github_tracker_roadmap_item (id, roadmap_id, issue_id, track, start_date, target_date, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            DatabaseEngine::Postgres => "INSERT INTO github_tracker_roadmap_item (id, roadmap_id, issue_id, track, start_date, target_date, sort_order, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
        };
        match self.pool().engine() {
            DatabaseEngine::Sqlite => { let pool = self.pool().as_sqlite().expect("sqlite pool"); sqlx::query(sql).bind(&id).bind(roadmap_id).bind(&command.issue_id).bind(&command.track).bind(&command.start_date).bind(&command.target_date).bind(sort_order).bind(&now).bind(&now).execute(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?; }
    ...
    async fn remove_tracker_roadmap_item(&self, _roadmap_id: &str, item_id: &str) -> Result<(), ServiceError> {
        let sql = match self.pool().engine() {
            DatabaseEngine::Sqlite => "DELETE FROM github_tracker_roadmap_item WHERE id = ?",
            DatabaseEngine::Postgres => "DELETE FROM github_tracker_roadmap_item WHERE id = $1",
        };
        match self.pool().engine() {
            DatabaseEngine::Sqlite => { let pool = self.pool().as_sqlite().expect("sqlite pool"); sqlx::query(sql).bind(item_id).execute(pool).await.map_err(|e| ServiceError::Repository(e.to_string()))?; }
